...
interface ScheduleEvent {
   ...
   * mais traité comme confirmé si absent/true, explicitement exclu si false :
   * on ne veut débloquer des fonds que sur du confirmé, jamais du
   * prévisionnel.
   */
  actual?: boolean;
}
...
/**
 * Dérive un `ContainerPositionStatus` à partir des codes d'événements de la
 * doc VesselFinder (section "Event-codes") : VDL = "Vessel departure from
 * first POL" (départ du port de chargement) → InTransit ; VAD = "Vessel
 * arrival at final POD" (arrivée au port de destination final) →
 * AtDestination. `schedule` est un historique cumulatif chronologique (rien
 * n'est jamais retiré au fil du voyage) : la présence de VAD implique
 * nécessairement que VDL est aussi présent, plus tôt dans la liste — d'où
 * une recherche par présence (`.some`) plutôt que par position/dernier
 * élément, qui serait fragile.
 */
...
/**
 * Interroge VesselFinder pour la position d'un conteneur. Retourne
 * `undefined` si aucune donnée exploitable n'est encore disponible
